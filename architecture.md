...
O projeto React estruturado em TypeScript seguirá o padrão abaixo:

```text
src/
├── assets/           # Ícones do PWA e imagens estáticas
├── components/       # Componentes de UI puros (botões, inputs, cards)
├── hooks/            # Custom hooks (uso do timer, estado do treino ativo)
├── services/         # Abstrações de I/O (localStorage, backup/JSON)
├── types/            # Definições de tipos e interfaces TypeScript
├── views/            # Telas principais (HistoryView, ActiveWorkoutView)
├── App.css           # Variáveis do tema claro e reset CSS
